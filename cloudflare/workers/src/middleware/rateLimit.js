...
async function getRateLimitData(c, clientId, windowMs) {
    const now = Date.now();
    const windowStart = now - windowMs;
    const windowStartIso = new Date(windowStart).toISOString();
    try {
        const result = await c.env.DB.prepare(`SELECT COUNT(*) as count FROM rate_limits
...
async function incrementRateLimit(c, clientId, windowMs) {
    try {
        await c.env.DB.prepare(`INSERT INTO rate_limits (client_id, created_at)
       VALUES (?, CURRENT_TIMESTAMP)`)
            .bind(clientId)
            .run();
        // Clean up old records (async, don't wait)
        c.executionCtx.waitUntil(c.env.DB.prepare(`DELETE FROM rate_limits WHERE created_at < datetime('now', '-${Math.ceil(windowMs / 1000)} seconds')`)
            .run());
    }
    catch (error) {
