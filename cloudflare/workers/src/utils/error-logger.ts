...
export class ErrorLogger {
  ...
  constructor(db: any, source: string = 'twilio-webhook') {
    this.db = db;
    this.source = source;
  }

  async logError(error: any, payload?: any): Promise<void> {
    try {
      const id = generateId();
  ...
  async logWarning(message: string, payload?: any): Promise<void> {
    try {
      const id = generateId();
      await this.db.prepare(`
  ...
  async logInfo(message: string, payload?: any): Promise<void> {
    try {
      const id = generateId();
      await this.db.prepare(`
        INSERT INTO system_logs (id, level, source, message, payload, created_at)
  ...
  static async logToSystem(db: any, level: string, source: string, message: string, payload?: any): Promise<void> {
    try {
      const id = generateId();
      await db.prepare(`
        INSERT INTO system_logs (id, level, source, message, payload, created_at)
