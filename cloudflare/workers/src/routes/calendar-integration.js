...
export const calendarIntegrationRoutes = new Hono();
function getDB(c) {
    const db = c.env.DB;
    if (!db) {
...
calendarIntegrationRoutes.get('/scheduled-calls', async (c) => {
    ...
    const stmt = db.prepare(`SELECT sc.*, p.organization_name, p.first_name, p.last_name, p.npi FROM scheduled_calls sc LEFT JOIN npi_providers p ON sc.provider_id = p.id ${whereClause} ORDER BY sc.scheduled_date ASC LIMIT 50`);
...
calendarIntegrationRoutes.post('/scheduled-calls', async (c) => {
    ...
    if (!scheduled_date)
        throw new HTTPException(400, { message: 'Scheduled date is required' });
    const db = getDB(c);
    const id = generateId();
...
calendarIntegrationRoutes.post('/sync/calendar', async (c) => {
    ...
    const id = generateId();
    // Create scheduled call
    let stmt = db.prepare(`INSERT INTO scheduled_calls (id, provider_id, contact_id, scheduled_date, timezone, status, created_by, created_at) VALUES (?, ?, ?, ?, 'pending', ?, ?)`);
    stmt = stmt.bind(id);
...
calendarIntegrationRoutes.put('/calendar/settings', async (c) => {
    ...
    let existingStmt = db.prepare('SELECT id FROM settings WHERE key = ? AND category = ?');
...
calendarIntegrationRoutes.get('/calendar/settings', async (c) => {
    ...
    for (const row of result.results || []) {
        const key = row.key.replace(`calendar_${user.id}_`, '');
...
calendarIntegrationRoutes.post('/availability/check', async (c) => {
    ...
    let stmt = db.prepare(`SELECT scheduled_date, timezone, status FROM scheduled_calls WHERE provider_id = ? AND status IN ('pending', 'confirmed')`);
    ...
    const conflicts = [];
    const existingDates = (existing.results || []).map((e) => e.scheduled_date);
