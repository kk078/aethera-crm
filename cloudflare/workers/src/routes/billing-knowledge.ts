...
export const billingKnowledgeBase: Array<{ topic: string; keywords: string[]; answer: string }> = [
  {
...
export function findBillingAnswer(query: string): Array<{ topic: string; answer: string; relevance: number }> {
  const lower = query.toLowerCase();
  const results: Array<{ topic: string; answer: string; relevance: number }> = [];
...
// ───── Specialty Objections Map ─────
// Used by AI call assistant to predict provider objections and suggest responses
...
export const specialtyObjections: Record<string, SpecialtyData> = {
  Cardiology: {
    valueProps: [
...
export function getSpecialtyData(specialty: string): SpecialtyData {
  const lower = specialty.toLowerCase();
  for (const [key, data] of Object.entries(specialtyObjections)) {
    if (lower.includes(key.toLowerCase())) return data;
    if (data.commonPainPoints.some(p => lower.includes(p.toLowerCase()))) return data;
  }
  return specialtyObjections['General Practice'];
}

export function getObjectionResponse(specialty: string, objection: string): string | null {
  const data = getSpecialtyData(specialty);
  const lower = objection.toLowerCase();
  for (const entry of data.objections) {
    if (lower.includes(entry.objection.toLowerCase().slice(0, 10))) return entry.response;
  }
  // Generic fallback
...
export function getValuePropositions(specialty: string): string[] {
  return getSpecialtyData(specialty).valueProps;
}

export function getCommonPainPoints(specialty: string): string[] {
  return getSpecialtyData(specialty).commonPainPoints;
}

// ───── Outreach Call Flow Phases ─────
// Used by the AI call assistant during live outbound calls
// Phase 1: Hook (0-30s), Phase 2: Discovery (2-5min), Phase 3: Value Proposition, Phase 4: Close

export interface OutreachPhase {
  id: string;
  label: string;
  strategy: string;
  default_opening?: string;
  specialty_openings?: Record<string, string>;
  pivot?: string;
  success_metric_mention?: string;
  detection_keywords: string[];
  questions?: string[];
}

export interface OutreachObjectionEntry {
...
export const outreachPhases: OutreachPhase[] = [
  {
    id: 'hook',
...
export const outreachObjectionMap: Record<string, OutreachObjectionEntry> = {
  we_do_in_house: {
...
export const specialtyKPIs: Record<string, { clean_claim: string; days_in_ar: string; net_collection: string }> = {
...
export const specialtyPowerPhrases: Record<string, string> = {
...
export function getSpecialtyKPI(specialty: string): { clean_claim: string; days_in_ar: string; net_collection: string } {
  const lower = specialty.toLowerCase();
...
export function getPowerPhrase(specialty: string): string {
  const lower = specialty.toLowerCase();
...
export function detectPhase(query: string, currentPhase: string, conversationLength: number): { phase: OutreachPhase; progress: number } {
  const lower = query.toLowerCase();
  ...
  if (conversationLength >= 3) {
    for (const kw of outreachPhases[3].detection_keywords) {
      if (lower.includes(kw)) return { phase: outreachPhases[3], progress: 100 };
    }
  }
  ...
  if (currentPhase === 'discovery' || currentPhase === 'value_proposition') {
    for (const kw of outreachPhases[2].detection_keywords) {
      if (lower.includes(kw)) return { phase: outreachPhases[2], progress: 75 };
    }
  }
  ...
  if (currentPhase === 'hook' || currentPhase === 'discovery') {
    for (const kw of outreachPhases[1].detection_keywords) {
      if (lower.includes(kw)) return { phase: outreachPhases[1], progress: 50 };
    }
  }

  // Advance based on conversation length
  if (conversationLength >= 2) return { phase: outreachPhases[2], progress: 75 };
  if (conversationLength >= 1) return { phase: outreachPhases[1], progress: 50 };

  return { phase: outreachPhases[0], progress: 25 };
}

export function matchObjection(query: string): OutreachObjectionEntry | null {
  const lower = query.toLowerCase();
...
export function getOutreachResponse(query: string, specialty: string, currentPhase: string, conversationLog: string[]): {
  phase: string;
  phase_label: string;
