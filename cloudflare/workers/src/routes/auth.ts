...
authRoutes.post('/login', async (c) => {
  try {
    ...
    const user: any = await ((c as any).env as any).DB.prepare(
...
authRoutes.get('/me', async (c) => {
  try {
    ...
    const userResult: any = await ((c as any).env as any).DB.prepare(
...
authRoutes.post('/setup', async (c) => {
  try {
    ...
    const existingUser: any = await ((c as any).env as any).DB.prepare(
