...
async function getTwilioClient(c: any): Promise<{ baseUrl: string; authHeader: string; accountSid: string }> {
...
twilioRoutes.post('/token', async (c) => {
  ...
  const accountSid = (c as any).env.TWILIO_ACCOUNT_SID;
  const apiKeySid = (c as any).env.TWILIO_API_KEY_SID;
  const apiKeySecret = (c as any).env.TWILIO_API_KEY_SECRET;

  if (!accountSid || !apiKeySid || !apiKeySecret) {
    throw new HTTPException(400, { message: 'Twilio Voice SDK not configured. Set API Key secrets.' });
  }

  const identity = (user as any).username || 'crm-rep';
...
async function sendSmsNotification(c: any, fromNumber: string, messageBody: string): Promise<void> {
  ...
  for (const to of NOTIFY_NUMBERS) {
    try {
      ...
      await fetch(`${baseUrl}/Accounts/${(c as any).env.TWILIO_ACCOUNT_SID}/Messages.json`, {
...
twilioRoutes.all('/voice', async (c) => {
  ...
  const fromNumber = (c as any).env.TWILIO_PHONE_NUMBER || '+18636940325';
...
twilioRoutes.post('/calls', async (c) => {
  ...
  const { baseUrl, authHeader } = await getTwilioClient(c);
  const fromNumber = from_number || (c as any).env.TWILIO_PHONE_NUMBER;
  if (!fromNumber) throw new HTTPException(400, { message: 'From number not configured' });
  ...
  const response = await fetch(`${baseUrl}/Accounts/${(c as any).env.TWILIO_ACCOUNT_SID}/Calls.json`, {
    method: 'POST', headers: { 'Authorization': authHeader, 'Content-Type': 'application/x-www-form-urlencoded' }, body: params,
  });
  if (!response.ok) {
...
twilioRoutes.post('/sms', async (c) => {
  ...
  if (!to_number || !messageBody) throw new HTTPException(400, { message: 'To number and message body are required' });
  const { baseUrl, authHeader } = await getTwilioClient(c);
  const fromNumber = (c as any).env.TWILIO_PHONE_NUMBER;
  if (!fromNumber) throw new HTTPException(400, { message: 'Twilio phone number not configured' });
  const params = new URLSearchParams();
  params.append('To', to_number); params.append('From', fromNumber); params.append('Body', messageBody);
  if (media_url) params.append('MediaUrl', media_url);
  const response = await fetch(`${baseUrl}/Accounts/${(c as any).env.TWILIO_ACCOUNT_SID}/Messages.json`, {
    method: 'POST', headers: { 'Authorization': authHeader, 'Content-Type': 'application/x-www-form-urlencoded' }, body: params,
  });
  if (!response.ok) { const error = await response.json(); throw new HTTPException(400, { message: `Twilio API error: ${(error as any).message}` }); }
  const message = await response.json();
  return c.json({ message: 'SMS sent successfully', data: { twilio_message_sid: (message as any).sid, status: (message as any).status } });
});
...
<Response>
  ...
  if (query) {
    ...
    if (results.length > 0) {
      ...
      if (results.length > 1) { responseText += ` I also found information about ${results.slice(1).map(r => r.topic).join(', ')}.`; }
    } else {
...
twilioRoutes.get('/recordings/:sid', async (c) => {
  const { sid } = c.req.param();
  const { baseUrl, authHeader } = await getTwilioClient(c);
  const response = await fetch(`${baseUrl}/Accounts/${(c as any).env.TWILIO_ACCOUNT_SID}/Recordings/${sid}.json`, { headers: { 'Authorization': authHeader } });
  if (!response.ok) throw new HTTPException(404, { message: 'Recording not found' });
...
twilioRoutes.get('/usage/stats', async (c) => {
  const { baseUrl, authHeader } = await getTwilioClient(c);
  const response = await fetch(`${baseUrl}/Accounts/${(c as any).env.TWILIO_ACCOUNT_SID}/Usage/Records/ThisMonth.json`, { headers: { 'Authorization': authHeader } });
