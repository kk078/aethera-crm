...
workflowsRoutes.get('/', async (c) => {
    ...
    const db = c.env.DB;
    let stmt = db.prepare(`SELECT COUNT(*) as total FROM workflows ${whereClause}`);
    ...
    stmt = db.prepare(`SELECT * FROM workflows ${whereClause} ORDER BY ${pagination.sort} ${pagination.order} LIMIT ? OFFSET ?`);
    for (const b of bindings) {
        stmt = stmt.bind(b);
    }
...
workflowsRoutes.get('/:id', async (c) => {
    ...
    const db = c.env.DB;
    let stmt = db.prepare('SELECT * FROM workflows WHERE id = ?');
    stmt = stmt.bind(id);
...
workflowsRoutes.put('/:id', async (c) => {
    ...
    const db = c.env.DB;
    let existingStmt = db.prepare('SELECT * FROM workflows WHERE id = ?');
    ...
    bindings.push(id);
    let stmt = db.prepare(`UPDATE workflows SET ${updates.join(', ')} WHERE id = ?`);
    for (const b of bindings) {
        stmt = stmt.bind(b);
    }
    await stmt.run();
...
workflowsRoutes.delete('/:id', async (c) => {
    ...
    let deleteStmt = db.prepare('DELETE FROM workflows WHERE id = ?');
