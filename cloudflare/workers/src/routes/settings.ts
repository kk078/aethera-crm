...
settingsRoutes.put('/:category/:key', async (c) => {
  ...
  const existing = await ((c as any).env as any).DB.prepare(
    'SELECT * FROM settings WHERE key = ? AND category = ?'
  )
    .bind(key, category)
    .first();
  ...
  const setting = await ((c as any).env as any).DB.prepare(
    'SELECT * FROM settings WHERE key = ? AND category = ?'
  )
    .bind(key, category)
    .first();
...
settingsRoutes.get('/integrations/:name', async (c) => {
  ...
  const integration = await ((c as any).env as any).DB.prepare(
...
settingsRoutes.put('/integrations/:name', async (c) => {
  ...
  if (existing) {
    await ((c as any).env as any).DB.prepare(
      'UPDATE integrations SET config = ?, is_active = ?, updated_at = CURRENT_TIMESTAMP WHERE name = ?'
    )
      .bind(JSON.stringify(config), is_active ? 1 : 0, name)
      .run();
  } else {
    await ((c as any).env as any).DB.prepare(
      'INSERT INTO integrations (id, name, type, config, is_active) VALUES (?, ?, ?, ?, ?)'
    )
