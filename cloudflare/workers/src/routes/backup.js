...
backupRoutes.get('/status', async (c) => {
    ...
    LIMIT 1
  `).first();
    const pendingBackups = await c.env.DB.prepare(`
...
backupRoutes.delete('/:id', async (c) => {
    ...
    const backup = await c.env.DB.prepare('SELECT * FROM backups WHERE id = ?').bind(id).first();
    if (!backup) {
    ...
    // Delete record
    await c.env.DB.prepare('DELETE FROM backups WHERE id = ?').bind(id).run();
    return c.json({
