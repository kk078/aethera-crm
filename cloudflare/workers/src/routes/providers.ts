...
import { createProviderSchema, updateProviderSchema, paginationSchema } from '../utils/validation';
import { generateId, calculatePagination, isValidNPI } from '../utils/helpers';
...
providersRoutes.get('/', async (c) => {
  ...
  let countStmt: any = db.prepare('SELECT COUNT(*) as total FROM npi_providers');
  const countResult = await countStmt.first();
  const total = countResult?.total || 0;
  console.log('Count result:', countResult);

  // Get paginated results - use D1's native limit/offset without bind params
  let listStmt: any = db.prepare(`SELECT id, npi, first_name, last_name, organization_name,
    specialty_primary, phone, email, city, state
    FROM npi_providers
    ORDER BY ${sortCol} ${sortOrder}`);
  const resultsRaw: any = await listStmt.all();
  const dataArray = Array.isArray(resultsRaw) ? resultsRaw : (resultsRaw as any).results || [];
  console.log('Results count:', dataArray.length);

  // Filter by email/phone filter (must have email or phone, not null and not empty)
  let filteredData = dataArray.filter((p: any) =>
    ((p.email !== null && p.email !== '') || (p.phone !== null && p.phone !== ''))
  );
  console.log('After email/phone filter count:', filteredData.length);

  // Filter by query params in memory
  if (query.specialty) {
    filteredData = filteredData.filter((p: any) => p.specialty_primary === query.specialty);
  }
  if (query.state) {
    filteredData = filteredData.filter((p: any) => p.state === query.state);
  }
  if (query.city) {
    filteredData = filteredData.filter((p: any) => p.city === query.city);
  }
  if (query.insurance) {
    filteredData = filteredData.filter((p: any) => p.insurance_panels && p.insurance_panels.includes(query.insurance));
  }
...
providersRoutes.get('/id/:id', async (c) => {
  ...
  const db = (c as any).env.DB as any;
  let stmt: any = db.prepare('SELECT * FROM npi_providers WHERE id = ?');
  stmt = stmt.bind(id);
  const provider = await stmt.first();
...
providersRoutes.post('/search/nppes', async (c) => {
  ...
  if (zipcode) params.append('postal_code', zipcode);
  ...
  try {
    ...
    const data: { results?: any[]; result_count?: number } = await response.json();
...
providersRoutes.post('/import', async (c) => {
  ...
  const db = (c as any).env.DB as any;
  let stmt: any = db.prepare('SELECT id FROM npi_providers WHERE npi = ?');
  ...
  if (!npi_data.basic) {
    ...
    try {
      ...
      const nppesData: { results?: any[] } = await nppesResponse.json();
  ...
  stmt = db.prepare('SELECT * FROM npi_providers WHERE id = ?') as any;
  stmt = stmt.bind(id);
  const provider = await stmt.first();
...
providersRoutes.put('/:id', async (c) => {
  ...
  const db = (c as any).env.DB as any;
  let existingStmt: any = db.prepare('SELECT * FROM npi_providers WHERE id = ?');
  ...
  let stmt: any = db.prepare(`UPDATE npi_providers SET ${updates.join(', ')} WHERE id = ?`);
...
providersRoutes.delete('/:id', async (c) => {
  ...
  let deleteStmt: any = db.prepare('DELETE FROM npi_providers WHERE id = ?');
...
providersRoutes.post('/claims/:id/verify', async (c) => {
  ...
  const db = (c as any).env.DB as any;
  let stmt: any = db.prepare('SELECT * FROM provider_claims WHERE id = ?');
  stmt = stmt.bind(id);
...
providersRoutes.get('/stats/summary', async (c) => {
  ...
  let bySpecialtyStmt: any = db.prepare(`
    ...
    LIMIT 10
  `);
  const bySpecialty: any = await bySpecialtyStmt.all();

  let byStateStmt: any = db.prepare(`
    ...
    LIMIT 10
  `);
  const byState: any = await byStateStmt.all();
