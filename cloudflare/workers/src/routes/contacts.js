...
contactsRoutes.get('/', async (c) => {
    ...
    const db = c.env.DB;
    // Get total count
    const countResult = await safeD1QuerySchemaAware(db, `SELECT COUNT(*) as total FROM contacts ${whereClause}`, [...bindings]);
    ...
    const queryResult = await safeD1QuerySchemaAware(db, `
...
contactsRoutes.get('/:id', async (c) => {
    ...
    const db = c.env.DB;
    const result = await safeD1QuerySchemaAware(db, `SELECT c.*, o.name as organization_name FROM contacts c LEFT JOIN organizations o ON c.organization_id = o.id WHERE c.id = ?`, [id]);
...
contactsRoutes.post('/', async (c) => {
    ...
    const tableCheck = await safeD1QuerySchemaAware(db, "SELECT name FROM sqlite_master WHERE type='table' AND name='contacts'", []);
    ...
    const contactResult = await safeD1QuerySchemaAware(db, `SELECT c.*, o.name as organization_name FROM contacts c LEFT JOIN organizations o ON c.organization_id = o.id WHERE c.id = ?`, [id]);
    return c.json({
...
contactsRoutes.put('/:id', async (c) => {
    ...
    const existingResult = await safeD1QuerySchemaAware(db, 'SELECT * FROM contacts WHERE id = ?', [id]);
