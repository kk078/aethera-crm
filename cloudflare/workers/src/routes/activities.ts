...
activitiesRoutes.get('/:id', async (c) => {
  ...
  const db = (c as any).env.DB as any;
  const stmt: any = db.prepare(`SELECT a.*, c.first_name, c.last_name, d.name as deal_name FROM activities a LEFT JOIN contacts c ON a.contact_id = c.id LEFT JOIN deals d ON a.deal_id = d.id WHERE a.id = ?`).bind(id);
...
activitiesRoutes.get('/contact/:contactId', async (c) => {
  ...
  const db = (c as any).env.DB as any;
  const activities = (db.prepare('SELECT * FROM activities WHERE contact_id = ? ORDER BY created_at DESC') as any).bind(contactId).all();
...
activitiesRoutes.get('/deal/:dealId', async (c) => {
  ...
  const db = (c as any).env.DB as any;
  const activities = (db.prepare('SELECT * FROM activities WHERE deal_id = ? ORDER BY created_at DESC') as any).bind(dealId).all();
  return c.json({ data: activities || [] });
});
