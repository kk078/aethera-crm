...
export const callQueueRoutes = new Hono();
function getDB(c) {
    const db = c.env.DB;
    if (!db) {
...
callQueueRoutes.get('/queue/items', async (c) => {
    ...
    const items = await db.prepare(`SELECT * FROM call_queue ${whereClause} ORDER BY created_at ASC LIMIT ? OFFSET ?`).bind(...queryBindings).all();
...
callQueueRoutes.patch('/queue/items/:id', async (c) => {
    ...
    const db = getDB(c);
    const stmt = db.prepare(`UPDATE call_queue SET ${status ? 'status = ?, ' : ''}${assigned_agent_id ? 'assigned_agent_id = ?, ' : ''}${intent ? 'intent = ?, ' : ''}${priority !== undefined ? 'priority = ?, ' : ''}updated_at = datetime('now') WHERE id = ?`);
    const bindings = [];
    if (status)
        bindings.push(status);
    if (assigned_agent_id)
        bindings.push(assigned_agent_id);
    if (intent)
