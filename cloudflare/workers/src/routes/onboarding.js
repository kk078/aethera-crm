...
import { generateId, calculatePagination, safeD1QuerySchemaAware, safeD1FirstSchemaAware, safeD1Execute } from '../utils/helpers';
...
function createStandardizedResponse(data, error, tableStatus) {
    return {
        data,
        error,
        meta: {
...
// Helper to check if table exists and return early if migration pending
async function checkTableExists(db, tableName) {
    const result = await safeD1QuerySchemaAware(db, `SELECT name FROM sqlite_master WHERE type='table' AND name='${tableName}'`, []);
...
async function safeQueryWithTableCheck(db, tableName, queryFn) {
    ...
    return { result: queryResult.data, status: 'ok' };
}
// Helper to create a task for provider technical setup automation
async function createProviderTask(c, providerId, title, description, priority = 'medium') {
    const db = c.env.DB;
...
async function completeRelatedTasks(c, providerId) {
    const db = c.env.DB;
    const user = c.get('user');
...
onboardingRoutes.get('/checklist/:dealId', async (c) => {
    ...
    const tableCheck = await safeD1QuerySchemaAware(db, "SELECT name FROM sqlite_master WHERE type='table' AND name='onboarding_checklists'", []);
    if (tableCheck.tableMissing) {
        return c.json(createStandardizedResponse(null, 'Database migration required', 'missing'), 200);
    }
    const checklist = await safeD1FirstSchemaAware(db, `SELECT oc.*, d.provider_id, d.name as deal_name
        FROM onboarding_checklists oc
        JOIN deals d ON oc.deal_id = d.id
        WHERE oc.deal_id = ?`, [dealId]);
...
onboardingRoutes.post('/checklist/:dealId', async (c) => {
    ...
    const dealResult = await safeD1FirstSchemaAware(db, 'SELECT id, provider_id FROM deals WHERE id = ?', [dealId]);
    ...
    const existingResult = await safeD1FirstSchemaAware(db, 'SELECT id FROM onboarding_checklists WHERE deal_id = ?', [dealId]);
    ...
    if (existingResult.data) {
        ...
        const checklistResult = await safeD1FirstSchemaAware(db, 'SELECT * FROM onboarding_checklists WHERE deal_id = ?', [dealId]);
        ...
        await safeD1Execute(db, `
            ...
            VALUES (?, ?, ?, 'in_progress')`, [checklistId, dealId, providerId]);
...
onboardingRoutes.get('/checklists', async (c) => {
    ...
    const db = c.env.DB;
    // Check if table exists first
    const tableCheck = await checkTableExists(db, 'onboarding_checklists');
    if (tableCheck === 'migration_pending') {
        return c.json(createStandardizedResponse([], 'Database migration required', 'missing'), 200);
    }
    // Get total count
    const countResult = await safeD1FirstSchemaAware(db, `SELECT COUNT(*) as total FROM onboarding_checklists oc ${whereClause}`, bindings);
    ...
    const result = await safeD1QuerySchemaAware(db, `
...
onboardingRoutes.put('/checklist/:dealId/items', async (c) => {
    try {
        ...
        const db = c.env.DB;
        // Verify deal exists
        const deal = await db.prepare('SELECT id, provider_id FROM deals WHERE id = ?').bind(dealId).first();
        if (!deal) {
            return c.json({ message: 'Deal not found', data: null }, 200);
        }
        const updates = ['overall_status = ?, updated_at = CURRENT_TIMESTAMP'];
        const bindings = ['in_progress', dealId];
        for (const [key, value] of Object.entries(body)) {
            if (value !== undefined && value !== null) {
                updates.push(`${key} = ?`);
                bindings.push(value);
...
onboardingRoutes.patch('/deals/:id/pipeline', async (c) => {
    ...
    const db = c.env.DB;
    // Check if table exists first
    const tableCheck = await checkTableExists(db, 'deals');
    if (tableCheck === 'migration_pending') {
        return c.json({ message: 'Database migration pending', status: 'migration_pending' }, 200);
    }
    // Get existing deal
    const existingResult = await safeD1FirstSchemaAware(db, 'SELECT * FROM deals WHERE id = ?', [id]);
    if (existingResult.tableMissing) {
        return c.json({ message: 'Database migration pending', status: 'migration_pending' }, 200);
    }
    if (!existingResult.data) {
    ...
    // Fetch updated deal
    const dealResult = await safeD1FirstSchemaAware(db, 'SELECT * FROM deals WHERE id = ?', [id]);
...
onboardingRoutes.get('/pipeline/summary', async (c) => {
    ...
    const counts = {};
    for (const stage of PIPELINE_STAGES) {
        const result = await safeD1FirstSchemaAware(db, `SELECT COUNT(*) as count FROM deals ${whereClause} AND pipeline_stage = ?`, [...bindings, stage]);
        counts[stage] = result.data?.count || 0;
    }
    // Also get onboarding stage counts
    const onboardingCounts = {};
    for (const stage of ONBOARDING_STAGES) {
        const result = await safeD1FirstSchemaAware(db, `SELECT COUNT(*) as count FROM deals ${whereClause} AND onboarding_stage = ?`, [...bindings, stage]);
...
onboardingRoutes.get('/providers/list', async (c) => {
    ...
    const db = c.env.DB;
    // Check if table exists first
    const tableCheck = await checkTableExists(db, 'npi_providers');
    if (tableCheck === 'migration_pending') {
        return c.json({
            data: [],
            pagination: {
    ...
    // Get total count
    const countResult = await safeD1FirstSchemaAware(db, `SELECT COUNT(*) as total FROM npi_providers p ${whereClause}`, bindings);
    if (countResult.tableMissing) {
        return c.json({
            data: [],
            pagination: {
    ...
    const providersResult = await safeD1QuerySchemaAware(db, `
...
onboardingRoutes.get('/dashboard/summary', async (c) => {
    ...
    const dealsCheck = await checkTableExists(db, 'deals');
    const checklistsCheck = await checkTableExists(db, 'onboarding_checklists');
    const tasksCheck = await checkTableExists(db, 'tasks');
    ...
    const pipelineCounts = {};
    for (const stage of PIPELINE_STAGES) {
        const result = await safeD1FirstSchemaAware(db, `SELECT COUNT(*) as count FROM deals d ${whereClauseSql} AND d.pipeline_stage = ?`, [...bindings, stage]);
        pipelineCounts[stage] = result.data?.count || 0;
    }
    // Onboarding checklists summary
    const checklistResult = await safeD1FirstSchemaAware(db, `
        SELECT
    ...
    const taskResult = await safeD1FirstSchemaAware(db, `
        ...
        `, bindings);
    // Revenue metrics
    const revenueResult = await safeD1FirstSchemaAware(db, `
        SELECT
...
onboardingRoutes.get('/document-vault/provider/:providerId', async (c) => {
    ...
    const db = c.env.DB;
    // Check if table exists first
    const tableCheck = await checkTableExists(db, 'document_vault');
    if (tableCheck === 'migration_pending') {
...
onboardingRoutes.post('/document-vault', async (c) => {
    ...
    const existingDeal = await safeD1FirstSchemaAware(db, 'SELECT id FROM deals WHERE provider_id = ? LIMIT 1', [provider_id]);
    ...
    const documentResult = await safeD1FirstSchemaAware(db, 'SELECT * FROM document_vault WHERE id = ?', [docId]);
...
onboardingRoutes.get('/bottlenecks', async (c) => {
    ...
    // Check if tables exist first
    const dealsCheck = await checkTableExists(db, 'deals');
    const providersCheck = await checkTableExists(db, 'npi_providers');
...
onboardingRoutes.get('/payer-enrollment/provider/:providerId', async (c) => {
    try {
        ...
        const db = c.env.DB;
        // Check if table exists first
        const tableCheck = await checkTableExists(db, 'payer_enrollment');
        if (tableCheck === 'migration_pending') {
...
onboardingRoutes.post('/payer-enrollment', async (c) => {
    try {
        ...
        const existing = await db.prepare('SELECT id FROM payer_enrollment WHERE npi_provider_id = ? AND payer_name = ?').bind(npi_provider_id, payer_name).first();
        if (existing) {
            // Update existing enrollment
            const updates = ['last_updated = CURRENT_TIMESTAMP'];
            const bindings = [npi_provider_id, payer_name];
            ...
            // Fetch updated
            return c.json({ message: 'Payer enrollment saved', data: await db.prepare('SELECT * FROM payer_enrollment WHERE id = ?').bind(existing.id).first() }, 201);
        }
        else {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
            bindValues(stmt, [enrollmentId, npi_provider_id, payer_name, payer_id || null, enrollment_status || 'pending', application_date || null, effective_date || null, contract_signed || 0, test_transactions_status || 'pending', production_status || 'pending']);
            await stmt.run();
            return c.json({ message: 'Payer enrollment saved', data: await db.prepare('SELECT * FROM payer_enrollment WHERE id = ?').bind(enrollmentId).first() }, 201);
        }
    }
    catch (error) {
...
onboardingRoutes.post('/payer-enrollment/provider/:providerId/bulk', async (c) => {
    ...
    `);
        bindValues(bulkStmt, [generateId(), providerId, carrier, status, providerId, carrier]);
        await bulkStmt.run();
    }
...
onboardingRoutes.get('/dashboard/revenue-readiness', async (c) => {
    try {
        ...
        const providersCheck = await checkTableExists(db, 'npi_providers');
        const dealsCheck = await checkTableExists(db, 'deals');
        const technicalSetupCheck = await checkTableExists(db, 'technical_setup');
...
onboardingRoutes.get('/compliance/provider/:providerId', async (c) => {
    ...
    const db = c.env.DB;
    // Check if table exists first
    const tableCheck = await checkTableExists(db, 'compliance');
    if (tableCheck === 'migration_pending') {
...
onboardingRoutes.post('/compliance/provider/:providerId', async (c) => {
    const { providerId } = c.req.param();
    const user = c.get('user');
    const body = await c.req.json();
    const db = c.env.DB;
...
onboardingRoutes.get('/technical-setup/provider/:providerId', async (c) => {
    try {
        ...
        const db = c.env.DB;
        // Check if table exists first
        const tableCheck = await checkTableExists(db, 'technical_setup');
        if (tableCheck === 'migration_pending') {
...
onboardingRoutes.post('/technical-setup/provider/:providerId', async (c) => {
    try {
        ...
        const db = c.env.DB;
        // Check if tables exist first
        const providersTableCheck = await checkTableExists(db, 'npi_providers');
        const technicalSetupTableCheck = await checkTableExists(db, 'technical_setup');
        ...
        if (existing) {
            ...
            if (edi_enrollment_status === 'active') {
                ...
                await createProviderTask(c, providerId, `Run Test Claim for ${providerName}`, `EDI enrollment is now active. Please run a test claim to verify the setup is working correctly.`, 'high');
            }
...
onboardingRoutes.get('/technical-setup/tracker', async (c) => {
    ...
    `).bind(...bindings).all();
        ...
        const completed = providers.filter((p) => p.setup_complete === 1).length;
        const inProgress = providers.filter((p) => p.setup_complete === 0 && (p.era_enrollment_status === 'pending' || p.edi_enrollment_status === 'pending')).length;
        const notStarted = providers.filter((p) => !p.clearinghouse_id).length;
        return c.json({
            providers,
            summary: {
                total,
...
onboardingRoutes.get('/report/revenue-readiness', async (c) => {
    ...
    `).all();
        ...
        const fullyReady = analysis.filter((a) => a.missingItems.length === 0).length;
        const needsAttention = analysis.filter((a) => a.missingItems.length > 0 && a.missingItems.length <= 2).length;
        const critical = analysis.filter((a) => a.missingItems.length > 2).length;
        // Count by state
        const byState = {};
        analysis.forEach((a) => {
...
onboardingRoutes.get('/audit/data-integrity', async (c) => {
    ...
    `).all();
        ...
        const uniqueProvidersWithIssues = Array.from(new Set(providersWithIssues.map((p) => p.id))).length;
        return c.json({
            audit_results: {
