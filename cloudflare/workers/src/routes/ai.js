...
aiRoutes.post('/call-preview', async (c) => {
    ...
    // Get call history
    const result = await c.env.DB.prepare(`SELECT id, status, duration, transcription, ai_summary, sentiment_score, created_at FROM phone_calls WHERE from_number LIKE ? OR to_number LIKE ? ORDER BY created_at DESC LIMIT 5`).bind(`%${cleanNumber.slice(-10)}%`, `%${cleanNumber.slice(-10)}%`).all();
...
aiRoutes.post('/predict/call-outcome', async (c) => {
    ...
    if (!caller_number)
        throw new HTTPException(400, { message: 'Caller number is required' });
    const db = c.env.DB;
    ...
    if (call_history && call_history.length > 0) {
        ...
        const negativeCalls = call_history.filter((h) => h.sentiment_score && h.sentiment_score < -0.2).length;
        if (negativeCalls > 1)
            sentimentTrend = 'negative';
        else if (negativeCalls === 1)
    ...
    `).bind(`%${cleanNumber.slice(-10)}%`, `%${cleanNumber.slice(-10)}%`).all();
        ...
        if (historyList.length > 0) {
            callTranscription = historyList.map((h) => h.transcription || '').join('\n');
            const negativeCalls = historyList.filter((h) => h.sentiment_score !== null && h.sentiment_score < -0.2).length;
            if (negativeCalls > 1)
                sentimentTrend = 'negative';
            else if (negativeCalls === 1)
    ...
    if (hasAIBinding(c) && callTranscription.length > 100) {
        try {
            ...
            if (prediction.recommended_timing)
                recommendedApproach = `AI recommends: ${prediction.recommended_timing}`;
        }
        catch { }
    }
    ...
    if (hasAIBinding(c)) {
        try {
          ...
          Return ONLY valid JSON.`
            ...
            if (call_id) {
                await c.env.DB.prepare('UPDATE phone_calls SET ai_summary = ?, sentiment_score = ? WHERE id = ? OR twilio_call_sid = ?').bind(JSON.stringify(summaryResult), 0.5, call_id, call_id).run();
            }
...
function getRuleBasedSummary(callTranscription, call_id, c) {
    const lines = callTranscription.split('.').filter((l) => l.trim().length > 5);
    const summary = lines.length > 0 ? `Call with ${lines.length} key points discussed. ${lines[0]}.` : 'Call transcript recorded.';
    const actionItems = [];
    const lower = callTranscription.toLowerCase();
    if (lower.includes('follow'))
    ...
    if (lower.includes('schedule') && lower.includes('call'))
        actionItems.push('Schedule a call');
    if (lower.includes('send') && lower.includes('info'))
        actionItems.push('Send information');
    if (lower.includes('pain') || lower.includes('problem') || lower.includes('issue') || lower.includes('struggling'))
        actionItems.push('Address pain points');
    if (actionItems.length === 0)
        actionItems.push('Review call transcript for action items');
    const sentiment = analyzeSentiment(callTranscription);
    return {
...
aiRoutes.get('/trend/sentiment/:number', async (c) => {
  ...
  `).bind(`%${cleanNumber.slice(-10)}%`, `%${cleanNumber.slice(-10)}%`).all();
    ...
    const sentiments = calls.map((call) => ({
        id: call.id,
    ...
    const scores = sentiments.filter((s) => s.score !== null).map((s) => s.score);
    ...
    let trend = 'no_data';
    if (scores.length >= 2) {
        const recentAverage = scores.slice(0, 3).reduce((a, b) => a + b, 0) / Math.min(3, scores.length);
        const olderAverage = scores.slice(3).reduce((a, b) => a + b, 0) / Math.max(1, scores.length - 3);
    ...
    return c.json({
        data: {
            ...
            positive_calls: sentiments.filter((s) => s.label === 'positive').length,
            negative_calls: sentiments.filter((s) => s.label === 'negative').length,
            neutral_calls: sentiments.filter((s) => s.label === 'neutral').length
        }
    });
});
