...
organizationsRoutes.get('/', async (c) => {
    ...
    const db = c.env.DB;
    let stmt = db.prepare(`SELECT COUNT(*) as total FROM organizations ${whereClause}`);
    ...
    stmt = db.prepare(`SELECT * FROM organizations ${whereClause} ORDER BY ${pagination.sort} ${pagination.order} LIMIT ? OFFSET ?`);
    for (const b of bindings) {
        stmt = stmt.bind(b);
    }
...
organizationsRoutes.get('/:id', async (c) => {
    ...
    const db = c.env.DB;
    let stmt = db.prepare('SELECT * FROM organizations WHERE id = ?');
    stmt = stmt.bind(id);
...
organizationsRoutes.put('/:id', async (c) => {
    ...
    const db = c.env.DB;
    let existingStmt = db.prepare('SELECT * FROM organizations WHERE id = ?');
    ...
    bindings.push(id);
    let stmt = db.prepare(`UPDATE organizations SET ${updates.join(', ')} WHERE id = ?`);
    for (const b of bindings) {
        stmt = stmt.bind(b);
    }
    await stmt.run();
    let auditStmt = db.prepare(`INSERT INTO audit_logs (user_id, action, table_name, record_id, old_values, new_values) VALUES (?, ?, ?, ?, ?, ?)`);
...
organizationsRoutes.delete('/:id', async (c) => {
    ...
    let deleteStmt = db.prepare('DELETE FROM organizations WHERE id = ?');
...
organizationsRoutes.get('/:id/contacts', async (c) => {
    const { id } = c.req.param();
    const db = c.env.DB;
    let stmt = db.prepare('SELECT * FROM contacts WHERE organization_id = ? ORDER BY last_name ASC');
    stmt = stmt.bind(id);
...
organizationsRoutes.get('/:id/deals', async (c) => {
    const { id } = c.req.param();
    const db = c.env.DB;
    let stmt = db.prepare('SELECT * FROM deals WHERE organization_id = ? ORDER BY created_at DESC');
    stmt = stmt.bind(id);
    const deals = await stmt.all();
    return c.json({
        data: deals.results || [],
    });
});
