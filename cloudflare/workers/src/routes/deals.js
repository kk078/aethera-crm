...
dealsRoutes.get('/', async (c) => {
    ...
    const db = c.env.DB;
    let stmt = db.prepare(`SELECT COUNT(*) as total FROM deals ${whereClause}`);
    for (const b of bindings) {
        stmt = stmt.bind(b);
    }
    const countResult = await stmt.first();
    const offset = (pagination.page - 1) * pagination.per_page;
    stmt = db.prepare(`SELECT d.*, c.first_name, c.last_name, o.name as organization_name FROM deals d LEFT JOIN contacts c ON d.contact_id = c.id LEFT JOIN organizations o ON d.organization_id = o.id ${whereClause} ORDER BY ${pagination.sort} ${pagination.order} LIMIT ? OFFSET ?`);
    for (const b of bindings) {
        stmt = stmt.bind(b);
    }
...
dealsRoutes.get('/pipeline/overview', async (c) => {
    ...
    const db = c.env.DB;
    let stmt = db.prepare(`SELECT stage, COUNT(*) as count, SUM(amount) as total_amount, AVG(probability) as avg_probability FROM deals ${whereClause} GROUP BY stage ORDER BY stage`);
    for (const b of bindings) {
        stmt = stmt.bind(b);
    }
    const pipeline = await stmt.all();
    return c.json({ data: pipeline.results || [] });
});
dealsRoutes.get('/:id', async (c) => {
    const { id } = c.req.param();
    const db = c.env.DB;
    let stmt = db.prepare(`SELECT d.*, c.first_name, c.last_name, c.email as contact_email, o.name as organization_name, o.type as organization_type FROM deals d LEFT JOIN contacts c ON d.contact_id = c.id LEFT JOIN organizations o ON d.organization_id = o.id WHERE d.id = ?`);
    stmt = stmt.bind(id);
...
dealsRoutes.post('/', async (c) => {
    ...
    const id = generateId();
    const db = c.env.DB;
    let stmt = db.prepare(`INSERT INTO deals (id, contact_id, organization_id, name, stage, amount, probability, expected_close_date, owner_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`);
    stmt = stmt.bind(id);
    ...
    stmt = stmt.bind(id);
    await stmt.run();
    stmt = db.prepare(`SELECT d.*, c.first_name, c.last_name, o.name as organization_name FROM deals d LEFT JOIN contacts c ON d.contact_id = c.id LEFT JOIN organizations o ON d.organization_id = o.id WHERE d.id = ?`);
    stmt = stmt.bind(id);
    const deal = await stmt.first();
...
dealsRoutes.put('/:id', async (c) => {
    ...
    const db = c.env.DB;
    let existingStmt = db.prepare('SELECT * FROM deals WHERE id = ?');
    ...
    bindings.push(id);
    let stmt = db.prepare(`UPDATE deals SET ${updates.join(', ')} WHERE id = ?`);
    for (const b of bindings) {
        stmt = stmt.bind(b);
    }
    await stmt.run();
...
dealsRoutes.delete('/:id', async (c) => {
    ...
    let deleteStmt = db.prepare('DELETE FROM deals WHERE id = ?');
...
dealsRoutes.patch('/:id/stage', async (c) => {
    ...
    const updates = ['stage = ?', 'updated_at = CURRENT_TIMESTAMP'];
    const bindings = [stage, id];
...
dealsRoutes.get('/:id/activities', async (c) => {
    const { id } = c.req.param();
    const db = c.env.DB;
    let stmt = db.prepare('SELECT * FROM activities WHERE deal_id = ? ORDER BY created_at DESC LIMIT 50');
    stmt = stmt.bind(id);
